...
- **Necessary PROSE edit (flagged deviation):** the spec said "keep PROSE unchanged" *and* "remove `list-disc`/`pl-6` from the `<ul>`" — these conflict, because `PROSE`'s descendant selectors `[&_ul]:list-disc [&_ul]:pl-6` re-applied disc + 24px indent to the new lists (the disc dot didn't show since the `<li>` is now `display:flex`, but the extra `pl-6` indent did). Removed **only** those two selectors from `PROSE` on both pages (kept `[&_ul]:mb-6 [&_ul]:space-y-2` + all `[&_p]`/`[&_li]` rules) so the square-marker lists render as specified.
