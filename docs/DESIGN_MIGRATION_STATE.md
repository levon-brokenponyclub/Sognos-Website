...
|---|------|-------------|---------|
| 1 | **Old edition tokens partially purged** | Colour system "complete, all built green" | `lime` and `pink` purged. **All 5 edition pages now use new `--sognos-edition-*` canonical tokens** (allied-health, disability, support-at-home, aged-care, child-and-family-services). Old `green/orange/coral/purple` tokens still live in `tokens.css` — referenced in: `Integration.tsx`, `industries/[slug]/page.tsx`, `industries/page.tsx`, `solutions/[slug]/page.tsx`, `products/page.tsx`, `style-guide/page.tsx`. Purge of old tokens blocked until those files migrate. |
...
| 3 | **Roster + Genogram Problems NOT dark** | "Roll SognosCare global sections onto Roster + Genogram" listed as pending | Both `sognosroster/Problems.tsx` and `sognosgenogram/Problems.tsx` use `bg-white` — no dark treatment applied. Confirmed pending. |
...
- **Navbar** — Aceternity pill + hover-pill: `rounded-full` pill bar, `layoutId="nav-hover-pill"` sliding highlight, dark-shadow `rounded-2xl` dropdown card with opacity/scale/y AnimatePresence. Dropdown morph reworked to **single content-fit panel** (out of `nav.map`): hidden measurer elements read natural widths at mount; `panelX` motion value spring-animates the x transform between items; `panelWidth` React state sets width synchronously in `useLayoutEffect` (avoids framer-motion v12's async resolver path for non-transform CSS); content crossfades via `AnimatePresence mode="wait"`. `variant: "light" | "dark"` prop seam; `THEMES` object separates all colours. Three-state scroll model retained.
...
| **ProductTrustStrip** | `components/sections/ProductTrustStrip.tsx` | done | Static 5-logo strip. `brightness-0 invert` for white logos. Reusable via `className` prop for any product dark bg. SognosCare wired; Roster/Genogram pending. |
...
| **SognosCare Hero** | `components/sections/sognoscare/Hero.tsx` | done | Scale zoom removed; parallax softened (`[0, -60]`); fade `[0.5,1]→[1,0.7]`; `pb-4 md:pb-0`. **AngelList pattern applied:** single `motion.div` wraps ALL hero content (eyebrow + headline + subtext + CTAs + panel); `<section>` owns the static `bg-sognos-care-dark` background (never transforms); panel is a plain child — no own motion. Fixes panel-detach/white-leak from the old per-panel transform. |
...
| **SognosCare EditionPageTemplate** | `components/sections/sognoscare/EditionPageTemplate.tsx` | done | Shared template: Hero/WhatItSolves/Features/Advantages/ProofStories |
| **Roster Hero** | `components/sections/sognosroster/Hero.tsx` | scaffolded | Not dark-pattern updated |
| **Roster Problems** | `components/sections/sognosroster/Problems.tsx` | styling-pending | `bg-white` — dark `bg-sognos-roster-dark` treatment PENDING |
| **Roster Features** | `components/sections/sognosroster/Features.tsx` | scaffolded | |
| **Roster Advantages** | `components/sections/sognosroster/Advantages.tsx` | scaffolded | |
| **Genogram Hero** | `components/sections/sognosgenogram/Hero.tsx` | scaffolded | |
| **Genogram Problems** | `components/sections/sognosgenogram/Problems.tsx` | styling-pending | `bg-white` — dark `bg-sognos-genogram-dark` treatment PENDING |
...
| `/knowledge-hub/[slug]` | 6 hardcoded posts, `twoCol: true` for sticky-meta layout |
| `/company/about` | `TeamSection`, `SocialResponsibilitySection` |
| `/company/careers` | `LifeAtSognos`, `OpenRoles` |
| `/company/social-responsibility` | `SocialResponsibilitySection` |
| `/contact` | `ContactForm` |
| `/customer-stories` | Hub (scaffolded) |
| `/customer-stories/[slug]` | Detail (scaffolded) |
...
| `--font-bureau-sans` | Unclear if Bureau Sans is licensed/loaded for production | Verify font loading in `app/layout.tsx` before launch |
...
| `components/layout/Navbar.tsx` | Aceternity pill navbar: floating `rounded-full` pill, `layoutId="nav-hover-pill"` hover highlight, single content-fit dropdown panel (measurer → `panelX` spring + `panelWidth` state), `variant` prop seam, 3-state scroll, mobile accordion |
