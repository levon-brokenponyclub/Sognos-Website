...
- **Navbar** — Aceternity pill + hover-pill: `rounded-full` pill bar, `layoutId="nav-hover-pill"` sliding highlight, dark-shadow `rounded-2xl` dropdown card. Dropdown: **persistent single `motion.div`** (no `AnimatePresence` wrapper — no re-mount on switch); `panelX` + `panelWidthMV` (both `useMotionValue`) spring simultaneously via `animate()` + `DROPDOWN_SPRING`; width clamped to `containerWidth - 2*EDGE`; `opacity`/`y` driven by `openMenu !== null`; `pointerEvents: "none"` when closed; `lastActiveGroupRef` preserves content during close fade. Content crossfades via inner `AnimatePresence mode="wait"` keyed on `openMenu ?? displayGroup.label`. Hover timers: 60ms open / 100ms close. `variant: "light" | "dark"` prop seam; `THEMES` object separates all colours. Three-state scroll model retained. Mobile: two-level full-screen slide (`AnimatePresence mode="wait"`, `mobilePanel` state, `mobilePanelDirectionRef` for direction).
