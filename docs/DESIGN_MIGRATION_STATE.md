...
| `/customer-stories/[slug]` | **Refactored 2026-07-02 — AngelList case-study layout, dark navy hero.** `bg-sognos-navy` hero with `lg:grid-cols-[200px_1fr]` sticky rail (back-link, "Case Study" category pill, `story.sidebar` Industry/State/Size/Product, dark-styled Download button, dark share icons) + main column (company label, white title, description subtitle, date/read-time, hero image w/ logo overlay). Route matched via `DARK_HERO_PATH_PREFIXES`. Quote restyled as a `rounded-lg bg-sognos-blue-accent` card (author/role split from `quoteAuthor` via local `parseQuoteAuthor`). Body prose restyled to match the Knowledge Hub article page. NEXT/prev replaced with a "Customer Stories" 3-up reusing `ArticleCard`, sourced via new `getCustomerStoryArchive()` query. |
