...
- **`ProductFeaturesScroll.tsx`** — sticky left rail with `motion.span layoutId="feature-rail-bullet"` spring pill + rAF-throttled scroll-spy (`getDocTop()` walk). Canonical scroll-spy rail pattern. `SolutionsSection.tsx` was refactored 2026-07-03 to the same shape (`layoutId="solutions-rail-bullet"`) — the previous tab-switcher/crossfade pattern is gone; if you need it back, reach for `AnimatePresence mode="wait"` + `motion.span layoutId="…"` from the pre-refactor version in git history. **`ArticleScrollNav.tsx`** (`components/layout/sections/shared/`, 2026-07-06 — moved/renamed from `customer-stories/StoryArticleNav.tsx`) is the **shared** scroll-spy rail for article pages (customer stories + knowledge hub): full-height `w-px bg-sognos-line` line + active **rectangle marker** `h-6 w-1 bg-sognos-blue-accent` (widened from `w-0.5` 2026-07-06), `layoutId="article-rail-marker"` spring 30/300, `getDocTop()` checkpoint `scrollY+140`. Heading helpers (`slugify`/`blockPlainText`/`extractHeadings`/`PortableBlock`/`ArticleSection`) now live in `lib/portableText.ts` (shared by both pages). `layoutId` must stay unique per component if two rails ever coexist on one page.
...
5 static slugs. Content from `lib/industries-content.ts`. **Full refactor 2026-07-06 (varied section rhythm, no card-monotony).** (1) **Hero** — matches the Solutions detail hero: `bg-white pt-32 pb-20 lg:pt-40 lg:pb-24`, `grid lg:grid-cols-12` left `col-span-7` = eyebrow `<Link href="/industries">{meta.name}</Link>` + `<h1>` (`text-5xl md:text-6xl lg:text-7xl`) + subtext (`text-gray-600`) + `<SolutionHeroDemoButton>`. No image. Old `bg-gradient-hero` / `data-header-dark` dark hero removed → navbar light theme. (2) **The Challenge** — stacking sticky cards via new `components/layout/sections/industries/IndustryChallengeStack.tsx` (Server Component; copied the AboutValues sticky mechanics — per-index `top = 80 + i*56`, `zIndex = (i+1)*10`, `rounded-t-lg` — **not** extracted to a shared component since the inner layout differs; handles 2–5 challenges). Cards alternate `bg-sognos-navy-dark`/`bg-sognos-navy`, number `01/02` (`text-white/30`), big title (`text-4xl lg:text-6xl`), body. Section eyebrow once above. (3) **How Sognos helps** — interactive tabs, new `industries/IndustryHowTabs.tsx` (`"use client"`): horizontal `role="tablist"` scroll-row (`scrollbar-hide`, `border-b border-sognos-line`), active tab `text-sognos-blue-accent` + spring `layoutId="how-tab-underline"` underline, content pane `AnimatePresence mode="wait"` opacity crossfade 0.25s, `lg:grid-cols-2` = white text card + empty `min-h-[220px]` bordered placeholder (no filler). Section `bg-gray-200/70`, title `text-sognos-heading`. (4) **What we deploy** — 2-col product feature cards (`lg:grid-cols-2 gap-3 lg:gap-4`), each `rounded-lg border border-sognos-line bg-white p-8 lg:p-10 hover:border-sognos-blue-accent/40`: full-colour logo (`/logos/sognos-{care,roster}-logo-color.svg`) + name + tagline + `PRODUCTS[].description` + "Explore {product} →". Energy special case: SognosCare slot → Microsoft Dynamics 365 card (Microsoft icon + tagline, no link). (5) **Customer stories** — new `industries/IndustryCustomerStories.tsx` (`"use client"` wrapper): filters `ALL_STORIES` by `industry === meta.name`, renders `ProductCustomerStories stories={…}`, **hides when none**. (6) Global `<CTABand>` (marketing layout) unchanged. **Data gaps flagged:** no per-product-per-industry bullets (`INDUSTRIES[].features` is industry-wide, not split by product) — used product description instead; `ALL_STORIES.industry` uses `"Transport"` for Auckland vs the normalized `"Facilities Management"`, so Facilities/Industrial stories don't match (sections hide) until reconciled.
...
| `/knowledge-hub/[slug]` | **Refactored 2026-07-06 to the customer-story pattern.** **Hero** (`bg-white`, `pt-32 lg:pt-40`, no pb): full-width back-link "Back to Knowledge Hub" above; then `lg:grid-cols-5` — **left `col-span-2`** = `border-l border-sognos-line pl-6` (line starts at the meta row, below the back-link) with an inline Amigo-style **meta row** (Category ` | ` `formatMetaDate` "Month D, YYYY" ` | ` read time, `text-xs uppercase tracking-widest text-sognos-muted`) + `<h1>` (`text-3xl lg:text-5xl`); **right `col-span-3`** = featured image `post.heroImage` `h-[60vh] lg:h-[80vh] object-cover object-top` (large fixed height so it breaks past the fold — crops bottom; empty column if no image, no placeholder). **Removed**: the old sticky `aside` (category badge / `BADGE_STYLES` / "Written by" author avatar / `ShareIcons`), excerpt paragraph, date-below-title line, and the below-title `aspect-[16/10]` hero image. **Body** (`lg:grid-cols-3`): col 1 = `lg:sticky lg:top-[100px]` `<ArticleScrollNav>` (shared — see below); col 2 = `max-w-[46rem]` Portable Text prose (`h2` serializer emits `id={slugify(text)}` + `scroll-mt`). **Latest articles** 3-up (`ArticleCard`) unchanged. Helpers now imported from `lib/portableText.ts`. |
...
| `/customer-stories/[slug]` | **Refactored 2026-07-02, extended 2026-07-05, Amigo-style 2026-07-06.** Server Component. Route matched via `DARK_HERO_PATH_PREFIXES` (transparent-over-navy navbar). Components under `components/layout/sections/customer-stories/`. **Hero** (`<HeroScrollFade>` client wrapper, `bg-sognos-navy`, scroll parallax/fade) — **rebuilt to amigo.ai layout 2026-07-06**: back-link + two-up `lg:grid-cols-[16rem_1fr]`. **Left rail** = hero-only vertical line (`border-l border-white/20 pl-6`, full-height via grid stretch, does NOT extend into body) with `Case Study` eyebrow (`text-white/60`) + Industry value (`text-white/80`). **Right** = `<h1>` (`text-3xl lg:text-5xl`) + **pull-quote** (`<blockquote>` `text-2xl md:text-3xl lg:text-4xl`, smart-quote glyphs) + `<figcaption>` attribution (`companyLogo` `brightness-0 invert` + `quoteAuthor`/`quoteRole` from `parseQuoteAuthor`). **Removed**: `story.description`, `story.heroImage`, the 3-col image/meta grid, the Industry/State/Size stats block, and the old end-of-body quote card (pull-quote replaces it) — with helpers `QUOTE_CARD_BG`/`quoteCardBg`/`brandBg`/`stats`/`category` + `BRAND_BG` import deleted. **Body** (`bg-white`, `lg:grid-cols-3`): **col 1** = `lg:sticky lg:top-[100px]` wrapper: (a) shared `<ArticleScrollNav>` scroll-spy (moved to `components/layout/sections/shared/` 2026-07-06) — vertical `w-px bg-sognos-line` rail + `h-6 w-1 bg-sognos-blue-accent` rectangle marker, `layoutId="article-rail-marker"` spring 30/300; heading helpers from `lib/portableText.ts`; (b) `bg-sognos-line` hairline; (c) `<StoryMetaRail>` (call-site now passes **State/Size/Product** only — Industry moved to hero, Customer dropped; component unchanged). **col 2** = `<ScrollReveal y={24}>` wrapping Portable Text prose (`max-w-[46rem]`). `h2` serializer emits `id={slugify(text)}` + `scroll-mt`. **Portable Text `calloutBlock`** (new 2026-07-06, `sanity/schemas/customerStory.ts`): `text` + `color` radio (orange/teal/blue/purple/coral); renderer `portableComponents.types.calloutBlock` = `w-full rounded-lg p-8 lg:p-10 font-heading text-xl lg:text-2xl text-white`, colour map `orange→bg-orange-600, teal→bg-teal-600, blue→bg-sognos-blue-accent, purple→bg-purple-600, coral→bg-rose-500`, **fallback `bg-sognos-blue-accent`**. Unfiltered `body` projection → no query change. **Bottom 3-up** = "What to read next" (`bg-gray-50`, h2 + `SeeMoreLink`, `ArticleCard` grid via `getCustomerStoryArchive()`). |
