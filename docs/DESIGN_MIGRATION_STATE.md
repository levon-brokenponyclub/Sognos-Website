...
| 2 | **`CTASection.tsx` file still exists** | Not mentioned | The component file `components/sections/CTASection.tsx` was NOT deleted — it was only removed from all page imports. The file remains on disk at 25 KB. |
...
| 4 | **`#10B981` hardcoded (5 spots, not 7)** | "~7 hardcoded spots" | Code grep finds exactly 5 occurrences, all in `components/sections/sognoscare/Features.tsx` inside the `FeatureVisual` placeholder components (which will be replaced by Lotties — so these may be moot). |
...
- **`SolutionsSection.tsx`** — animated tab-switcher using `AnimatePresence` + crossfade panels + `motion.span layoutId="subnav-pill"` moving pill. This is the canonical pattern for animated selection states.
- **`ProductFeaturesScroll.tsx`** — sticky left rail with `motion.span layoutId="feature-rail-bullet"` spring pill + rAF-throttled scroll-spy (`getDocTop()` walk).
...
| **Hero (homepage)** | `components/sections/Hero.tsx` | done | **Reworked to navy.** `bg-sognos-navy-dark`; two-card panel + `useScroll`/parallax removed; `pt-40 pb-20` text-only block; heading/subtext → `text-white`/`text-white/70`; primary CTA → `AnimatedButton variant="white"`; secondary link → `text-white/70 border-white/30`. Stagger fadeUp entrance retained. Cards below peek above fold at load. |
| **HomeProductCards** | `components/sections/HomeProductCards.tsx` | styling-pending | **AngelList column layout + mobile slider.** Navy bg (`bg-sognos-navy-dark pb-24`) seamless with hero. **Mobile**: `flex snap-x snap-mandatory overflow-x-auto scroll-px-6 scrollbar-hide gap-3`; each card `min-w-[82%] shrink-0 snap-start` (one card fills 82% viewport, next peeks). **Desktop (`md`+)**: `md:grid md:grid-cols-3 md:overflow-visible lg:gap-4`; cards reset `md:min-w-0 md:shrink`. Per column: top + bottom hairline (`border-white/15`), `pt-4 pb-6`. Structure: h3 title → `aspect-[3/4] rounded-lg` image card (bg image opacity-20, dark overlay, logo top-left, arrow button bottom-right) → always-visible description `mt-5 text-lg text-white/70`. Hover = arrow button accent fill only (no glow). Accents: Care `#1d96fc`, Roster `#59bbf7`, Genogram `#91278c`. FLAG: Genogram bg = Care placeholder. FLAG: dot indicators — noted as refinement. |
| **LogoStrip** | `components/sections/LogoStrip.tsx` | done | Sanity-backed infinite CSS marquee; `trust-marquee-track` CSS animation; do NOT modify |
| **HowSognosWorks** | `components/sections/HowSognosWorks.tsx` | done | 3-block "Safe. Flexible. Independent." |
| **IndustrySection** | `components/sections/IndustrySection.tsx` | done | Arrow snap-scroll square cards; Industrial Services video autoplays always |
| **SolutionsSection** | `components/sections/SolutionsSection.tsx` | done | Animated tab-switcher, `motion.span layoutId`, `AnimatePresence`. **Canonical motion reference.** |
| **NewsInsightSection** | `components/sections/NewsInsightSection.tsx` | done | 3 callout cards with morphing SVG notch (`.callout-card__svg`), Framer Motion drag slider |
| **CTABand** | `components/sections/CTABand.tsx` | done | Globally rendered in marketing layout above `<Footer />`. Book a Demo button `disabled` — interim until `/book-demo` page built. `useBookDemo` modal wiring removed. |
| **CTASection** | `components/sections/CTASection.tsx` | scaffolded | File exists (25 KB) but removed from all page imports. Retained for now; can be deleted later. |
| **ProductTrustStrip** | `components/sections/ProductTrustStrip.tsx` | done | Static 5-logo strip. Always `bg-white`, `grayscale opacity-70` logos. Wired to SognosCare, SognosRoster, and Sognos Genogram pages (after Hero, wrapped in `<ScrollReveal>`). **Dividers added (2026-07-02):** `border-l border-gray-200` on each logo wrapper except first (`index > 0`), hidden on mobile, `md:flex-1 md:px-10` per-logo wrapper for breathing room. `divide-x` utility was skipped (Tailwind v4 child-selector issue); per-item conditional class used instead. |
| **ProductFeaturesScroll** | `components/sections/ProductFeaturesScroll.tsx` | styling-pending | Side-by-side cards (text left / `PlaceholderBox` right). Stacking removed — normal `space-y-8 lg:space-y-12` flow. Sticky left rail + scroll-spy intact. Lottie wiring pending export-size decision. |
...
| Route | Notes |
|-------|-------|
| `/knowledge-hub` | `KnowledgeHubArchive` — 6 articles, sticky filters |
...
| `/company/about` | **Refactored 2026-07-02 — AngelList engineering/careers layout.** (1) Hero: `bg-sognos-navy` two-up (h1 left / intro+CTA right). Route added to `DARK_HERO_PATHS`. (2) About section: `bg-white` two-col `lg:grid-cols-[200px_1fr]` — eyebrow left, h2 + 4 body paragraphs + `<AboutStats />` right. (3) `<AboutValues />` — full-bleed stacking sticky cards (Mission blue-accent top:80, Vision navy top:136, `rounded-t-lg`, no buffer, flows into Beliefs). (4) `<AboutBeliefs />` — static Server Component, single `bg-white/5` card, 3 values. Unchanged: `<TeamSection />`, Partners, `<SocialResponsibilitySection />`. |
...
| `/customer-stories/[slug]` | **Refactored 2026-07-02 — AngelList case-study layout, dark navy hero (revised same day).** Server Component. Route matched via `DARK_HERO_PATH_PREFIXES` (transparent-over-navy navbar). **Hero** = `<HeroScrollFade>` client wrapper (`relative overflow-hidden bg-sognos-navy`) applying SognosCare-style scroll parallax/fade (`useScroll` `["start start","end start"]`, `y:[0,1]→[0,160]`, `opacity:[0,0.7]→[1,0]`, reduced-motion guard) over the static navy bg. Hero grid `lg:grid-cols-[200px_1fr]`: left rail = back-link + Category pill + Product + Download (conditional) + Share (dark); main column = `flex flex-col lg:min-h-[440px]` with logo/title/date at top and **Industry/State/Size** stat row anchored bottom via `lg:mt-auto`. **No hero image.** **Body** = white section, same `[200px_1fr]` grid: left = `<StoryMetaRail>` (client, `lg:sticky lg:top-[104px]`, `whileInView` fade/slide-in) holding Customer Name → intro (`description`) → stacked Industry/State/Size/Product → Download (only if `downloadUrl` set) → Share; right = quote card (product-mapped dark tone, `ProductCustomerStories`-style body, author/role/logo, no image) + Portable Text prose (`max-w-[46rem]`). NEXT/prev replaced with "Customer Stories" 3-up reusing `ArticleCard` via `getCustomerStoryArchive()`. New components: `components/sections/customer-stories/StoryMetaRail.tsx`, `HeroScrollFade.tsx`. Sanity `Industry` values normalized to canonical taxonomy (published). Quote card bg now driven by Sanity `brandColor.hex` field (falls back to hardcoded `lib/customerStoryBrand.ts` map, then product-dark tone) — 2026-07-03. |
...
| `components/sections/CTABand.tsx` | Global "Book a Demo" band with dual logo marquees. `disabled` button. |
| `components/sections/ProductTrustStrip.tsx` | Reusable trust strip (5 white logos, static). Used on product pages. |
| `components/sections/ProductFeaturesScroll.tsx` | Features scroll: sticky left rail + scroll-spy + side-by-side cards. Lottie slot pending. |
| `components/ui/PlaceholderBox.tsx` | Self-measuring Lottie placeholder with live W×H display. Temporary. |
| `components/sections/SolutionsSection.tsx` | Animated tab-switcher. **Canonical motion reference.** |
| `components/sections/sognoscare/Hero.tsx` | SognosCare hero — AngelList pattern: static dark section bg, single motion.div wraps all content |
| `components/sections/sognoscare/Problems.tsx` | Lumos-style light section: problem→solution blocks + 5 capability columns |
| `components/sections/sognoscare/Advantages.tsx` | AngelList checklist — dark bg, framer stagger |
| `components/sections/sognoscare/EditionCards.tsx` | Editions slider — AngelList fund-admin card pattern (scaffold); `accentColor` hex prop (token migration pending) |
| `components/sections/sognoscare/EditionPageTemplate.tsx` | Shared template for all 6 edition pages |
